...
```bash
terraform init -migrate-state # migrate the local terraform.tfstate to remote backend
aws s3 ls s3://workshop-tfstate-<account-id> # check contents on s3 bucket, to see that there is a state file
terraform destroy # cleanup (this deletes the example.txt file)
```
